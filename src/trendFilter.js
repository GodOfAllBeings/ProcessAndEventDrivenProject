...
function removeOutdatedFiles() {
  let oldDate = new Date(new Date().setDate(new Date().getDate() - 6))
    .toJSON()
    .slice(0, 10);
  ...
  trendFileNames.forEach((fileName) => {
    ...
    if (isDateTooOld(fileDate, oldDate)) {
      try {
        ...
        console.log(
          `Removed file ${trendFilePath}/${fileName} because ${fileDate} is before ${oldDate}`
        );
      } catch (err) {
        console.error(err);
      }
...
function isDateTooOld(fileDate, oldDate) {
  fileDateFormat = new Date(
    fileDate.substring(0, 4),
    fileDate.substring(5, 7),
  ...
  return (
    new Date(fileDateFormat.toDateString()) <
    new Date(oldDateFormat.toDateString())
  );
}
...
function getBestTrends() {
  ...
  jsonTrends.forEach((jsonObj) => {
    let trend = jsonObj["trendingSearchTitle"]["searchTitle"];
    if (trendMap.has(trend)) {
      trendMap.set(trend, trendMap.get(trend) + 1);
    } else {
      trendMap.set(trend, 1);
    }
  });
